...
const GuessTextPage = ({ navigation, route }) => {
  ...
  function createGuessOptions(dummies, ingredients) {
    ...
    const dummyNumber =
      Math.floor((Math.random() * ingredients.length) / 2) + 2;
    ...
    if (dummyNumber < dummyCopy.length) {
      ...
      const updatedList = combinedList.map((n, index) => {
        return { name: n, id: index };
      });
  ...
  async function checkAndSubmitAnswers() {
    ...
    for (let i = 0; i < recipeData.ingredients.length; i++) {
      if (
        answers.find((n) => n.name === recipeData.ingredients[i]) != undefined
      ) {
        rightAnswers++;
      }
    }
      ...
      <View style={styles.container} ref={animatedRef}>
        <View style={styles.answerOptions}>
          <Text style={styles.titleText}>
            {" "}
            Drag items from a {recipeData.name} to green
          </Text>
