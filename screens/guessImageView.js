...
const GuessImagePage = ({ navigation, route }) => {
  ...
  useEffect(() => {
    ...
    const mixedArray = createImageList(
      totalDummies,
      guessOptions.numberOfImages
    );
    try {
  ...
  async function downloadImages(imageData) {
    ...
    for (let image of imageData) {
      if (image.type === "dummy") {
        ...
        imageRef = ref(
          storage,
          `ingredient-${locationId}-${guessOptions.id}-${image.id}.jpg`
        );
      }
      const singleImage = await getDownloadURL(imageRef);
      imageList = [
        ...imageList,
        { url: singleImage, id: counter++, type: image.type, visible: true },
      ];
    }
  ...
  function createImageList(totalDummies, numberOfImages) {
    ...
    return [...dummyNumberList, ...ingredientsNumberList].sort(
      (a, b) => 0.5 - Math.random()
    );
  }
  ...
  function moveToTrash(image) {
    if (image.visible) {
      ...
      setGuessImages(
        guessImages.map((n) =>
          n.id === image.id ? { ...n, visible: false } : n
        )
      );
    }
  }
  ...
  function undoTrash() {
    if (trash.length > 0) {
      ...
      setGuessImages(
        guessImages.map((n) =>
          n.id === firstIndex.id ? { ...n, visible: true } : n
        )
      );
  ...
  const GuessItem = ({ image, onMove }) => {
    ...
    const onGestureEvent = useAnimatedGestureHandler({
      ...
      onEnd: (event) => {
        const coordinates = getRelativeCoords(
          animatedRef,
          event.absoluteX,
          event.absoluteY
        );
    ...
    return (
      <PanGestureHandler onGestureEvent={onGestureEvent}>
        <Animated.View style={[animateStyle]}>
          <View
            style={
              image.visible ? styles.answerOptions : styles.answerInvisible
            }
          >
    ...
    <GestureHandlerRootView style={styles.rootView}>
      <View style={styles.container} ref={animatedRef}>
        <Text style={styles.titleText}>
          {" "}
          Drag items not in a {guessOptions.name} to trash
        </Text>
